...
public class RuleData<C> {
     ...
     * created.
     */
    public RuleData(int xDimension, int yDimension, C defaultCellContent, AgentBoolean rec) {
        RuleDataAction.x = xDimension;
        RuleDataAction.y = yDimension;
        this.rec = rec;
     ...
     */
    AgentBoolean rec = null;

    public Cell<C> result(Cell<C> s, RuleDataAction a) {
        int x = a.getXResult(s.getX());
        int y = a.getYResult(s.getY());
        Cell<C> sDelta = getCellAt(x, y);
        rec.initCol();
        if (null == sDelta || !rec.isAction(x) || !rec.isAction(y) || !rec.isAction(s.getX()) || !rec.isAction(s.getY())) {
