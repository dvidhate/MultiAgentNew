package learningalgo.search.online;

import learningalgo.search.framework.ActionsFunction;
import learningalgo.search.framework.DefaultStepCostFunction;
import learningalgo.search.framework.GoalTest;
import learningalgo.search.framework.StepCostFunction;
...
public class OnlineSearchProblem {
     ...
     *
     * @param actionsFunction ACTIONS(s), which returns a list of actions
     * allowed in state s
     * @param goalTest GOAL-TEST(s), which the agent can apply to a single state
     ...
     * @param goalTest GOAL-TEST(s), which the agent can apply to a single state
     * description to determine if it is a goal state
     * @param stepCostFunction the step-cost function c(s, a, s') - note that
     * this cannot be used until the agent knows that s' is the outcome
     */
    public OnlineSearchProblem(ActionsFunction actionsFunction,
     ...
     *
     * @param state an object representing a state
     *
