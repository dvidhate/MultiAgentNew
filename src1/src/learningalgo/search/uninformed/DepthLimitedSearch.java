...
import learningalgo.search.framework.CutOffIndicatorAction;
import learningalgo.search.framework.Node;
import learningalgo.search.framework.NodeExpander;
import learningalgo.search.framework.Problem;
import learningalgo.search.framework.Search;
import learningalgo.search.framework.SearchUtils;
...
public class DepthLimitedSearch extends NodeExpander implements Search {
     ...
     *
     * @param result the action list resulting from a previous search
     *
   ...
   // function DEPTH-LIMITED-SEARCH(problem, limit) returns a solution, or
     ...
     * goal you will receive a List with a single NoOp Action in it. If fail to
     * find the Goal, an empty list will be returned to indicate that the search
     * failed. If the search was cutoff (i.e. reached its limit without finding
     * a goal) a List with one CutOffIndicatorAction.CUT_OFF in it will be
     * returned (Note: this is a NoOp action).
     */
     ...
     *
     * @param pathCost the value of the path cost metric
     */
