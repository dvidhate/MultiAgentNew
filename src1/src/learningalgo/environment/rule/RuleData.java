 ...
 *
 *
 * @param <C> the type of content for the Cells in the world.
 *
 *
 *
 */
public class RuleData<C> {

    private Set<Rile<C>> cells = new LinkedHashSet<Rile<C>>();
    private Map<Integer, Map<Integer, Rile<C>>> cellLookup = new HashMap<Integer, Map<Integer, Rile<C>>>();

    /**
     * Construct a Cell World with size xDimension * y Dimension cells, all with
     * their values set to a default content value.
     *
     * @param xDimension the size of the x dimension.
     * @param yDimension the size of the y dimension.
     *
     * @param defaultCellContent the default content to assign to each cell
     * created.
     */
    public RuleData(int xDimension, int yDimension, C defaultCellContent) {
        for (int x = 1; x <= xDimension; x++) {
            Map<Integer, Rile<C>> xCol = new HashMap<Integer, Rile<C>>();
            for (int y = 1; y <= yDimension; y++) {
                Rile<C> c = new Rile<C>(x, y, defaultCellContent);
                cells.add(c);
                xCol.put(y, c);
            }
     ...
     *
     * @param s the cell location from which the action is to be performed.
     * @param a the action to perform (Up, Down, Left, or Right).
     * @return the Cell an agent would end up in if they performed the specified
     * action from the specified cell location.
     */
    public Rile<C> result(Rile<C> s, RuleDataAction a) {
        Rile<C> sDelta = getCellAt(a.getXResult(s.getX()), a.getYResult(s
                .getY()));
        if (null == sDelta) {
     ...
     *
     * @param x the x dimension of the cell to be removed.
     * @param y the y dimension of the cell to be removed.
     */
    public void removeCell(int x, int y) {
        Map<Integer, Rile<C>> xCol = cellLookup.get(x);
        if (null != xCol) {
     ...
     *
     * @param x the x dimension of the cell to be retrieved.
     * @param y the y dimension of the cell to be retrieved.
     * @return the cell at the specified x,y location, null if no cell exists at
     * this location.
     */
    public Rile<C> getCellAt(int x, int y) {
        Rile<C> c = null;
        Map<Integer, Rile<C>> xCol = cellLookup.get(x);
        if (null != xCol) {
            c = xCol.get(y);
