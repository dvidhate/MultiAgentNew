...
public class apriori {
    ...
    public static ArrayList header;
//-------------------------------------------------------------
...
//System.out.println("Now reading transactions, increment counters of itemset");
            ...
            Vector vt = (Vector) (largeitemset.elementAt(k - 1));
            ht.put(String.valueOf(k), concertVectorLabel(vt));
            System.out.println(concertVectorLabel(vt));
        }
...
//System.out.println("End.");
    ...
    Vector concertVectorLabel(Vector vt) {
        Vector newlabel = new Vector();
        for (int i = 0; i < vt.size(); i++) {
            String l = vt.elementAt(i).toString();
            String newl = "";
            StringTokenizer st = new StringTokenizer(l, " ");
            while (st.hasMoreTokens()) {
                newl += header.get(Integer.parseInt(st.nextToken()) - 1);
                if (st.hasMoreTokens()) {
                    newl += " ";
                }
            }
