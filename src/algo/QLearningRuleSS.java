...
public class QLearningRuleSS extends Graph {

    public double R[][];

    public int currentState = 0;

    public QLearningRuleSS() {
        initData();
    }

    public QLearningRuleSS(int Q_SIZE, int[] INITIAL_STATES, double[][] R) {
...
//        }
    ...
    public double qupdate(final int action) {
        ...
        if (cslearning) {
            return findSimleSTWij(qupdate, currentState, action);
        } else {
            updateVal(qupdate);
            return (qupdate);
        }
